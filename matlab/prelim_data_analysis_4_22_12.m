...
ratedCapacity = PV_area*peakPower; %Rated PV capacity under ideal conditions 
...
demandMet = 0;
n = 1000;
MaxCapacityNorm = zeros(length(peakHours),n);
LOLP = zeros(length(peakHours),n);% Loss of Load Probability = number of battery capacity 
...
for kx = 1:length(peakHours)
    ...
    for ixx = 1:n;
        ...
        LOLP(kx,ixx) = sum(batteryCharge == 0)/length(batteryCharge);
        %if ixx ==1
        %   figure
    ...
    MaxCapacityNorm(kx,:) = MaxCapacity/ratedCapacity(kx); % Capacity in terms of hours of peak generation
    % peak generation is defined as the rated capacity of the PV module
end
figure
plot(MaxCapacityNorm',LOLP')
axis tight
xlabel('Battery Storage Capacity (hr)') % Capacity in terms of hours of peak generation
ylabel('Loss of Load Probablity')
title('LOLP vs Normalized Maximum Battery Capacity')
legend('PV Capacity 744 W','580 W','464 W','387 W') 
