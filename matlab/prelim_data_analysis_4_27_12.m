...
peakPower = 1000;
ratedCapacity = PV_area*peakPower; 
%% System Simulation
...
demandMet = 0;
n = 10000;
MaxCapacity = linspace((max(batteryCharge1)-min(batteryCharge1))/10,10000,n);%(max(batteryCharge1)-min(batteryCharge1)),n);
...
MaxCapacityNorm = MaxCapacity/ratedCapacity; %
% chopOff = min(find(LOLP == 0));
% LOLP = LOLP(1:chopOff);
% MaxCapacity = MaxCapacity(1:chopOff);
% MaxCapacityNorm = MaxCapacityNorm(1:chopOff);
% WhShort = WhShort(1:chopOff);

figure
plot(MaxCapacity,LOLP)
axis tight
xlabel('Battery Storage Capacity (Whr)')
